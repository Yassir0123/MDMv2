...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────
export default function UserSubordinatesPageRH() {
   ...
   // ── INITIAL LOAD ────────────────────────────────────────────────
   useEffect(() => {
      if (currentUser) fetchSubordinates()
   }, [currentUser])
   ...
   const fetchSubordinates = async () => {
      ...
      try {
         ...
         allUsers.forEach(u => { if (u.id != null) map[u.id] = u })
         setUsersById(map)
         const managerIds = new Set(allUsers.filter(u => u.isManager === 1).map(u => u.id))
         const list = allUsers.filter(u => u.managerId && managerIds.has(u.managerId))
               ...
               <table className="w-full min-w-[860px]">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                     {loading ? (
                        ...
                        const manager = sub.managerId ? usersById[sub.managerId] : undefined
                        return (
   ...
   {manager ? (
      <div>
         <p className="text-xs font-bold text-slate-700">{manager.nom} {manager.prenom}</p>
         <p className="text-[10px] text-slate-400 font-mono">{manager.matricule || "—"}</p>
      </div>
   ) : (
